...
public class EasyRun {
	...
	public static void main(String[] args) {
		...
		// 추상클래스 객체 생성 불가 (비완성 클래스라서~)
		// Sports s = new Sports(); 
		// Sports s; // 객체 생성이 안될 뿐이지 레퍼런스로는 사용 가능
		Sports s = new BasketBall(); // 다형성 적용 시켜서 부모타입으로 자식객체 받아주는 형태가능
		
		Sports[] arr = new Sports[2]; // 배열만 만든거
		
		//부모				자식
 		arr[0] = new BasketBall(); // 다형성 적용
 		arr[1] = new FootBall();
 		...
 		/*
 		 * 동적바인딩 : 컴파일 시점(코드 막 입력하는 시점)에는 정적바인딩으로 현재 레퍼런스의 자료형의
 		 * 			 클래스에 있는 메소드를 가리키긴함
 		 * 			 런타임시점(실행시점)에서는 동적바인딩으로 각각의 자식클래스에 오버라이딩 된
 		 * 			 메소드가 있을경우 그게 실행됨
 		 * 
 		 */
 		 ...
 		 * 미완성된 클래스임.
 		 * 일반필드 + 일반메소드 [+ 추상메소드] -> 추메없어도 추클가능
 		 * -> 추상메소드를 가진 클래스는 반드시 추상클래스로 명시!!
 		 * 
 		 * 추상메소드가 굳이 없어도 추상클래스로 만들 수 있음
 		 * 언제사용되나요?
 		 * 
 		 * > 개념적 : 단지 이 클래스가 미완성된 클래스다 라는걸 부여할 목적 => 덜 만들었을 때
 		 ...
 		 * 
 		 * => 각 자식클래스마다 실행시킬 내용은 다르지만 동일한 형태의 메소드로 구현했으면할때
 		 * 		=> 메소드 통일성 확보 목적
 		 * 		=> 표준화된 틀을 제공할 목적
 		 * 
 		 */
