		...
		while (flag == true) { // 메뉴 스위치가 켜져있는 동안 무한반복
			...
			case 1: // 1. 조회하기
				...
				if (search == 1) {
					...
					for(Book b : bc.onlySearchBook())
					System.out.println(b);
				}
				else if (search == 3) {
					for(Book b : bc.onlySearchMagazine())
						System.out.println(b);
				}
				else if (search == 4) {
					continue;
			...
			case 3:// 3. 책 찾기
				...
				if (search == 1) { // bNo으로 책 찾기
					...
					ArrayList<Book> arr = new ArrayList<>();
					int year = sc.nextInt();
					arr = bc.magazineOfThisYearInfo(year);
					...
					arr = bc.searchBookByPublisher(publisher);
					...
					price = sc.nextInt();
					sc.nextLine();
					arr = bc.searchBookByPrice(price);
					if(arr.size() == 0) {
						System.out.println("조회된 도서가 없습니다.");	
					} else {
						for(Book b:arr) {
			...
			case 4:// 4. 전체책 가격 합계 및 평균 조회
				...
				System.out.println(bc.getTotalPrice() + "원");
				System.out.print("전체책 가격 평균 : " );
				System.out.println(bc.getAvgPrice() + "원");
				break;
