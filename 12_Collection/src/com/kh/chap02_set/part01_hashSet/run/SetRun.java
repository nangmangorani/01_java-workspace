...
public class SetRun {
	...
	public static void main(String[] args) {
		...
		hs1.add(new String("너무 보고싶었어요호호"));
		hs1.add(new String("여러분"));
		System.out.println(hs1); // 저장순서가 유지 안됨!(인덱스 개념 없음), 중복된 데이터(동일객체) 보관 불가능
...
//		System.out.println(new Student("공유",43,100).equals(new Student("공유",43,100)));
		...
		// 1. for문 사용가능 (단, 향상된 for문(for each문)만 사용 가능!)
		for(Student s : hs2) {
		...
		// ArrayList에 담는 첫번째 방법 : ArrayList 생성 후 addAll 메소드를 이용해서 통째로 추가하기
		...
		// 3. Iterator 반복자 이용해 순차적으로 접근하는 방법
		Iterator<Student> it = hs2.iterator(); // hs2에 담겨있는 객체들을 Iterator(반복자)에 담는 과정
		while(it.hasNext()) {
			Student s = it.next(); // student 객체 반환
			System.out.println(s);
		}
		// it.next(); 더이상 뽑을거없어서 에러
	}
