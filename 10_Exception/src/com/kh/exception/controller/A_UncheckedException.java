...
public class A_UncheckedException {
	 ...
	 * ArithmeticException : 나누기 연산기 0으로 나누려 할 때 발생되는 예외
	...
	public void method1() {
		...
		// 애초에 예외 자체가 발생이 안되게끔 if 문으로 조건 검사 -> 엄연히 예외처리는 아님
		/*
		int result = 0;
		if(num2 != 0) {
			result = num1 / num2;
		...
		*/
		...
		// 예외처리 : 예외가 "발생했을 때" 실행할 내용을 미리 작성해놓은 방법
		...
		try {
			...
			System.out.println("result : " + result);
		}catch(ArithmeticException e) { // 예외를 낚아채버려
			System.out.println("0으로 나눌 수 없습니다.");
			e.printStackTrace(); // 강제로 오류난 이력을 보고자할때 실행하는 구문
		}
	...
	public void method2() {
		...
		// NegativeArraySizeException : 배열 할당하는 과정에서 배열의 크기를 음수로 지정하는 경우 발생되는 예외
	...
	public void method3() {
		...
		try {
			...
			int[] arr = new int[size];
			System.out.println("100번 인덱스 값 : " + arr[100]);
		}catch(InputMismatchException e) { // 다형성 적용해서 부모타입 예외클래스 작성 가능
									 // 모든 자식 예외 발생시 다 받아서 처리할 수 있음. 순서 매우중요!!!!!!!!!
			System.out.println("정수가 아닌 값을 입력하셨습니다.");
		} catch (RuntimeException e) {
			System.out.println("예외가 발생되긴했음.. 배열의 크기가 잘못됐던가, 부적절한 인덱스가 제시됐던가, 정수로 입력을 안했던가..");
		}
			/*
		try {
			int size = sc.nextInt();
			...
			System.out.println("부적절한 인덱스로 접근했습니다.");
		}catch(InputMismatchException e) { // java.util이라 import해줘야함.
			System.out.println("정수로 입력해야됩니다.");
		}
		*/
		// 부모예외클래스와 자식예외클래스를 가지고 catch 블럭을 기술하고자 할 때는
		// 자식예외클래스 catch 블럭이 위에 있어야함!! (아니면 unreachable catch block 오류뜸)
		
		/*
		 * RuntimeException 관련애들은 UnCheckedException (예외처리구문이 필수는 아님)
		 * if문 : 애초에 예외자체가 발생되기 전에 소스코드로 핸들링 하는거임 (예외처리구문이 아님)
		 * try - catch문 : 예외가 "발생했을경우" 처리해주는 구문을 작성해두는 것(예외처리구문)
		 * 
