...
public class InetRun {
	...
	public static void main(String[] args) {
		...
		/*
		 * 네트워크(Network) : 여러대의 컴퓨터들이 연결되어있는 통신망같은거
		 * 					=> 네트워크를 통해 서로간 데이터 교환 가능
		 * IP 주소 : 네트워크 상에 각 컴퓨터들을 식별해줄 수 있는 번호(컴퓨터의 주민번호)
		 * 
		 * 서버와 클라이언트
		 * 서버 		: 클라이언트(고객)에게 서비스를 제공해주는 프로그램 (클라이언트의 요청을 처리해서 응답해주는) : 서빙하는 역할
		...
		try {
			...
			InetAddress googleHost = InetAddress.getByName("www.google.com"); // 도메인명을 통해 해당 호스트에 대한 정보반환
		...
		/*
		 * 현재 구동중인 서버가 있으면
		 * 클라이언트 그 서버로 요청보낼 수 있음 => 응답결과 돌려줌
		 * 
		 * 요청과 응답에 의해서 프로그램은 돌아감 ("통신"한다 표현)
		 * 웹에서 통신방식 : HTTP 프토로콜 통신
		 * 
		 * 자바를 가지고 서버와 클라이언트 간의 간단한 통신 해보기!
		 * 이때 데이터를 입출력하고자 한다면 서버와 클라이언트 간의 스트림(연결통로)이 필요함 
		 * 
		 * 소켓 : 프로세스간의 통신을 담당하는 것(스트림을 연결하기 위한 문)
		 * 
		 * 소켓 프로그래밍 (TCP 방식 / UDP 방식)
		 * TCP 방식 : 데이터의 전송속도가 느림! 데이터가 정확하고 안정적 전달 가능! (신뢰성이 요구되는 프로그램 : http, 이메일)
		 * UDP 방식 : 데이터 전송속도가 빠름! 신뢰성없는 데이터가 전달될 수 있음! (데이터를 빠른 속도로 전송하고자 하는 프로그램 : 실시간 동영상 서비스)
		 * 
		 */
