...
public class ServerProgram {
	...
	public static void main(String[] args) {
		...
		/*
		 * TCP(Transmission Control Protocol) 서버, 클라이언트 간 1:1 소켓 통신 데이터를 교환하기에 앞서 서버,
		 * 클라이언트가 연결되어있어야함! (서버가 먼저 실행되어 클라이언트 요청 기다릴거임) 신뢰성 있는 데이터 전달 가능
		 * 
		 * 소켓(Socket) 프로세서간의 통신을 담당 InputStream / OutputStream을 가지고 있음 (이 스트림을 통해 입출력
		 * 이루어짐)
		 * 
		...
		PrintWriter pw = null; // 한줄 씩 읽어오는 출력보조스트림

		// 1) 포트 번호 지정 (서버측에서 몇 번 포트로 통로를 열꺼냐)
		int port = 3000; // 0~1023은 컴터에서 쓰는경우많음

		ServerSocket server = null;

		try {
			// 2) Server Socket 객체 생성시 포트 결합(bing) => (ServerSocket은 클라이언트 연결요청 받아줄 용도의 소켓)
			server = new ServerSocket(port);
			// 3) 클라이언트로부터 접속 요청 올때까지 대기 상태..
			...
			// PrintWriter : 데이터 출력 print(), println() 메소드 갖고 있는 보조스트림

			while (true) {
				// 7) 스트림을 통해 읽고 쓰기
				// 클라이언트로부터 전달된 메세지가 있을 경우 서버측 읽어들일거임! (입력)
				String message = br.readLine(); // 반환형 확인!!
				...
				pw.flush(); // 현재 스트림에 있는 데이터를 강제로 내보내는 메소드 => close() 자동 호출
			}
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
			// 8) 통신 종료, 가장 늦게 만든거부터 닫으면됨!!
			pw.close();
			try {
				br.close();
				server.close();
